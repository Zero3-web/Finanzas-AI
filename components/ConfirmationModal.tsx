...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({ isOpen, onClose, onConfirm, title, message, confirmText = 'Delete', cancelText = 'Cancel' }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 transition-opacity duration-300" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div className="bg-surface dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md m-4 transform transition-all duration-300 scale-95 animate-modal-in">
        <div className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-text-main dark:text-gray-100" id="modal-title">{title}</h3>
            <div className="mt-2">
        ...
        <div className="bg-secondary dark:bg-gray-700/50 px-4 py-3 sm:px-6 flex flex-row-reverse rounded-b-2xl">
            <button
                type="button"
            ...
            <button
                type="button"
                className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-surface dark:bg-gray-800 text-base font-medium text-text-main dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:mt-0 sm:w-auto sm:text-sm transition-colors"
                onClick={onClose}
            >
                {cancelText}
            </button>
