...
const ViewSwitcher: React.FC<ViewSwitcherProps> = ({ options, value, onChange }) => {
  ...
  return (
    <div className="relative flex items-center bg-gray-200 dark:bg-secondary-dark rounded-full p-1 w-full max-w-xs mx-auto">
      <div
      ...
      {options.map((option) => (
        ...
        >
          <div className={`transition-colors duration-300 ${value === option.value ? 'text-white' : 'text-text-secondary dark:text-text-secondary-dark'}`}>
            {option.icon}
          </div>
        </button>
      ))}
