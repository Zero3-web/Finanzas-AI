...
const TransactionForm: React.FC<TransactionFormProps> = ({ accounts, onAddTransaction, onUpdateTransaction, onClose, transactionToEdit, t }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-text-secondary dark:text-gray-400">{t('transactionType')}</label>
        <div className="mt-1 flex rounded-md shadow-sm">
            <button type="button" onClick={() => setType(TransactionType.EXPENSE)} className={`w-full px-4 py-2 rounded-l-md transition-colors ${type === TransactionType.EXPENSE ? 'bg-expense text-white' : 'bg-secondary hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600'}`}>{t('expense')}</button>
            <button type="button" onClick={() => setType(TransactionType.INCOME)} className={`w-full px-4 py-2 rounded-r-md transition-colors ${type === TransactionType.INCOME ? 'bg-income text-white' : 'bg-secondary hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600'}`}>{t('income')}</button>
        </div>
      </div>
      <div>
        <label htmlFor="accountId" className="block text-sm font-medium text-text-secondary dark:text-gray-400">{t('account')}</label>
      ...
      <div>
        <label htmlFor="description" className="block text-sm font-medium text-text-secondary dark:text-gray-400">{t('description')}</label>
      ...
      {isCreditCardPayment && (
         <div>
            <label htmlFor="installments" className="block text-sm font-medium text-text-secondary dark:text-gray-400">{t('installments')}</label>
      ...
      <div>
        <label htmlFor="date" className="block text-sm font-medium text-text-secondary dark:text-gray-400">{t('date')}</label>
